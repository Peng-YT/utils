 ...
 * @returns
 */
export const getRemarkHtmlByKeyword = (html, keyword, matchClassName = 'red', startIndex = 0) => {
    if (!keyword.trim()) {
        return {
            match: false,
            html,
    ...
    const reg2 = new RegExp(keyword.split('').map((word, idx) => {
    ...
    }).join(''), 'gi'); // 匹配非text标签的html
    const hasMatch2 = html.match(reg2);
    if (hasMatch2) {
        return {
            match: true,
            html: html.replaceAll(reg2, `$1<span class="${matchClassName}">$2</span>$3`),
